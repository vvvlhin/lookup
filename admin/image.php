...
<body>
  <div class="auth">
    <div class="main">
      <div class="flex-main">
        <h3 id="main-header-text">Добавление изображения</h3>
      ...
      <form method="post" id="form-img" enctype="multipart/form-data">
        <div id="message"></div>
        <h1 for="first" id="input-text">Наименование изображения<span id="red">*</span>
        </h1>
        <input type="text" id="input" class="name" required>
        <h1 for="password" id="input-text">
          Изображение <span id="red">*</span>
        </h1>
        <input class="file" type="file" name="uploadfile" value="" />
        <div class="wrap">
          <!-- <input type="submit" class="sub" value="Добавить"> -->
          <input class="sub btn-primary" type="submit" value="Добавить"></input>
        </div>
      </form>
    </div>
  </div>
  <div class="blur">
    ...
    <aside class="sidebar-nav-wrapper">
      ...
      <nav class="sidebar-nav">
        <ul>
          ...
          <li class="nav-item">
            <a href="../admin/image.php?page=0" data-bs-toggle="collapse" data-bs-target="#ddmenu_2"
              aria-controls="ddmenu_2" aria-expanded="false" aria-label="Toggle navigation">
              <span class="icon">
    ...
    <main class="main-wrapper">
      ...
      <section class="section">
        <h1 class="preview-text" style="padding: 25px 50px;">Список изображений
        ...
        <div class="table-section">
          <table>
            ...
            <tbody class="tbody">
              ...
              $res_count = mysqli_query($connect, "SELECT COUNT(*) FROM card_img");
              if (isset($_GET['page'])) {
                $page = $_GET['page'];
              } else {
                $page = 1;
              ...
              $sql = mysqli_query($connect, "SELECT * FROM card_img ORDER BY Code_img DESC LIMIT " . $number . ", " . $limit . "");
              $products = array();
              ...
              <? foreach ($products as $key => $product): ?>
                <tr class="row__group">
                  <td id="id" style="text-align: center;"><?= $product['Code_img'] ?></td>
                  <td>
                    <a target="_blank"
                      href="images.php?pic=<?php echo urlencode($product["Name_img"]); ?>"><?= $product["synonym"] ?></a>
                  </td>
                  <td><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" id="edit-img"
